...
### Gap AM — Plant smog in view (plan slice G5) **shipped**

The strategy audit found the opening a trap. The coach said to place the plant "beside the street", and the natural spot is beside the first houses. A plant's smog (60 at the stack, 8 tiles) plus factories next door (their smog adds up) drove homes out, so the naive town stalled at 200–390 people for six years, against 700. Nothing warned before the plant went down.

| Slice | What shipped |
|---|---|
| AM1 Smog reach | `sim/smogReach.ts` spreads a would-be source's smog exactly as PollutionSystem does. It finds the homes and shops it would reach (at least 10 smog) and those it would push past the level that drives them out (60, counting the smog already there). |
| AM2 Plant tool | Hovering the Plant tool tints those lots (light brown where reached, dark where driven out) beside the power reach, and the status line says so: "Plant here: its smog would reach 30 homes and 13 shop lots, enough to drive 17 out — place it further from the houses (smog carries 8 tiles)." Away from town it reads "no homes or shops in its smog". The placement note repeats the warning, judged before the plant went down. |
| AM3 Factory areas | Dragging an industrial zone area tints the homes and shops its first factories' smog would reach, and adds "its factories' smog would reach 8 homes…" to the status. |
| AM4 Coach | The opening advice says to place the plant beside a street, away from the houses (its smog carries 8 tiles). |

The scripted player gets a `heedSmog` policy. The new `naive-heeds` strategy is the naive player following the warnings: plants go where their smog reaches the fewest homes (the first one at the end of a road out of town) and factories go in their own district. It reaches 724 people at year 10, against 276 for the naive player and 700 for the balanced one, whose first plant still sits in town. `test/strategyBalance.ts` holds the band: at least 0.8× balanced and 1.5× naive.

**Exit:** In `?city=hamlet`, pick the Plant tool and hover beside the crossroads. The homes around turn brown and the status line counts them.
