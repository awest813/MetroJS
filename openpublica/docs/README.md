...
- [x] Placement: road tools drag straight or L-shaped lines with a live cost/bridge preview (Esc cancels, Shift paints freehand), and upgrading a street pays only the difference; the cursor turns red where the tool would refuse; buildings face their street; cars curve through turns in their lane
- [x] Zoning: zone brushes drag rectangles with a live preview; deep areas lay their own streets, long blocks closed into loops (S toggles), lots with no street show amber; each month's new buildings follow demand and fill outward from existing ones
